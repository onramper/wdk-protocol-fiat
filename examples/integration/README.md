...
| **Checkout / session** | `getTransactionDetail` (bootstrap → DPoP-bound envelope → checkout session) | a session (see below) |
...
`getTransactionDetail` is the one session-gated call. It resolves a checkout session
...
**session (session)** access token and presents it as an envelope on the
checkout call:

1. **Your backend mints a session.** One request signing-signed call to
   `POST /partners/v2/{apiKey}/client-sessions` returns `{ sessionId, sessionToken }`.
   The `sessionToken` (`st_…`) is **single-use**.
2. **The SDK exchanges it** for a session access token (`at_…`) directly against
   `POST /partners/v2/{apiKey}/client-sessions/tokens`, with a DPoP proof, an
   `X-Onramper-Channel`, and `attestation: { type: 'none' }`. No attestation or
   device challenge — that is what *session* means.
...
The exchange in step 2 only succeeds if the partner has session sessions enabled,
with the channel allowed and the SDK's requested scope/TTL within the ceilings
(both are clamped down to these, never up):

```jsonc
// partner config → security.sessions.session
{
  "channels": ["wdk-web"],                          // must include the SDK's channel
  "scope": ["read", "write"],  // requested scope is clamped to this
  "token TTL": 900                                   // token TTL is clamped to this
}
```
...
| `ONRAMPER_CHECKOUT_SESSION_ID` | a checkout session session id to look up (from an intent) |
